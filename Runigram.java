...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		int rows = image.length;
		int cols = image[0].length;
		Color [][] imcopy1 = new Color[rows][cols];
		for (int i = 0 ; i < rows ; i++) {
			for (int j = 0 ; j < cols ; j++) {
				imcopy1 [i][j] = image[i][cols - 1 - j]; 
			}
		}
		//// Replace the following statement with your code
		return imcopy1;
	...
	public static Color[][] flippedVertically(Color[][] image){
		int rows = image.length;
		int cols = image[0].length;
		Color [][] imcopy2 = new Color[rows][cols];
		for (int i = 0 ; i < rows ; i++) {
			for (int j = 0 ; j < cols ; j++) {
				imcopy2 [i][j] = image[rows - 1 - i][j]; 
			}
		}
		//// Replace the following statement with your code
		return imcopy2;
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		Color [][] sColors = new Color[height][width];
		for (int i = 0; i < height;i++) {
			for (int j = 0; j < width ; j++) {
				int newh = i * h0 / height;
				int neww = j * w0 / width;
				sColors [i][j] = image [newh][neww];
